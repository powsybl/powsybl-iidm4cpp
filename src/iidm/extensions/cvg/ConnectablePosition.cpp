...
template <>
const std::initializer_list<std::string>& getNames<powsybl::iidm::extensions::cvg::ConnectablePosition::Direction>() {
...
void ConnectablePosition::check(const OptionalFeeder& feeder, const OptionalFeeder& feeder1, const OptionalFeeder& feeder2, const OptionalFeeder& feeder3) {
    if (!feeder && !feeder1 && !feeder2 && !feeder3) {
        throw PowsyblException(logging::format("invalid feeder"));
    }
    if (feeder && (feeder1 || feeder2 || feeder3)) {
        throw PowsyblException(logging::format("feeder and feeder 1|2|3 are exclusives"));
    }
    bool error = false;
    if (feeder1) {
        if (!feeder2 && feeder3) {
    ...
    if (error) {
        throw PowsyblException(logging::format("feeder 1|2|3 have to be set in the right order"));
    }
}
