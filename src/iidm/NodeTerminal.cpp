...
void NodeTerminal::traverse(VoltageLevel::TopologyTraverser& traverser) {
    dynamic_cast<NodeBreakerVoltageLevel&>(getVoltageLevel()).traverse(*this, traverser);
}

void NodeTerminal::traverse(VoltageLevel::TopologyTraverser& traverser, std::vector<std::reference_wrapper<Terminal>>& traversedTerminals) {
