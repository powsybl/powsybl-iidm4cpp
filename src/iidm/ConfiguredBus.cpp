...
double ConfiguredBus::getP() const {
    double p = 0.0;

    const std::list<std::reference_wrapper<BusTerminal> >& terminals = m_terminals[m_network.get().getStateIndex()];
    for (const auto& it : terminals) {
        BusTerminal& terminal = it.get();
        if (terminal.isConnected()) {
            ...
            switch (connectable.getConnectableType()) {
                ...
                default:
                    ...
                    oss << "Unexpected ConnectableType value:" << connectable.getConnectableType();
...
double ConfiguredBus::getQ() const {
    double q = 0.0;
