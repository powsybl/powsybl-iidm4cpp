...
void checkPhaseTapChangerRegulation(const Validable& validable, const PhaseTapChanger::RegulationMode& regulationMode, double regulationValue, bool regulating,
    ...
    if (regulationTerminal && !stdcxx::areSame(regulationTerminal->getVoltageLevel().getNetwork(), network)) {
...
void checkRatioTapChangerRegulation(const Validable& validable, bool loadTapChangingCapabilities, bool regulating, const stdcxx::Reference<Terminal>& regulationTerminal, double targetV, const Network& network) {
    if (loadTapChangingCapabilities && regulating) {
        ...
        if (!stdcxx::areSame(regulationTerminal->getVoltageLevel().getNetwork(), network)) {
