...
bool NodeBreakerVoltageLevel::connect(Terminal& terminal) {
    ...
    node_breaker_voltage_level::Graph::VertexVisitor isBusbarSection = [](const stdcxx::Reference<NodeTerminal>& term) {
        return static_cast<bool>(term) && term.get().getConnectable().get().getType() == ConnectableType::BUSBAR_SECTION;
    };
