...
static std::array<std::string, 249>& getCountryCodes() {
    static std::array<std::string, 249> s_countryCodes = {{
        u8"AF",
        u8"AX",
        u8"AL",
        u8"DZ",
...
std::string getCountryCode(const Country& country) {
    return logging::toString(getCountryCodes(), country);
}

Country getCountryFromCode(const std::string& countryCode) {
    const auto& countryCodes = getCountryCodes();
    const auto& it = std::find(countryCodes.cbegin(), countryCodes.cend(), countryCode);
    if (it == countryCodes.cend()) {
        throw PowsyblException(logging::format("Unable to retrieve country from ISO code '%1%'", countryCode));
    }
    return static_cast<Country>(it - countryCodes.cbegin());
}
