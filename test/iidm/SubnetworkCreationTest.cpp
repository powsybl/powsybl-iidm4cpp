...
BOOST_AUTO_TEST_CASE(moveNetworkWithSubnetworksAndVoltageAngleLimits) {
    Network network = CreateSubnetworksNetworkTest();
    Network& subnetwork1 = network.getSubNetwork("Sub1").get();
    Network& subnetwork2 = network.getSubNetwork("Sub2").get();
    ...
    Network movedNetwork = std::move(network);

    BOOST_CHECK_EQUAL(2, movedNetwork.getSubNetworksCount());
    BOOST_CHECK_EQUAL(5, boost::size(movedNetwork.getVoltageAngleLimits()));
    BOOST_TEST(stdcxx::areSame(movedNetwork, subnetwork1.getParentNetwork()));
    BOOST_TEST(stdcxx::areSame(movedNetwork, subnetwork2.getParentNetwork()));
    BOOST_CHECK_EQUAL(1,  boost::size(subnetwork1.getVoltageAngleLimits()));
    BOOST_CHECK_EQUAL(1,  boost::size(subnetwork2.getVoltageAngleLimits()));

    BOOST_CHECK(stdcxx::areSame(vla0, movedNetwork.getVoltageAngleLimit("vla0")));
    BOOST_CHECK(stdcxx::areSame(vla1, movedNetwork.getVoltageAngleLimit("vla1")));
    BOOST_CHECK(stdcxx::areSame(vla1, subnetwork1.getVoltageAngleLimit("vla1")));
