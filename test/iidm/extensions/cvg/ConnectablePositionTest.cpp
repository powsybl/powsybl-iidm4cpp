...
#include <powsybl/iidm/extensions/cvg/ConnectablePosition.hpp>
...
BOOST_FIXTURE_TEST_CASE(ConnectablePositionXmlSerializerFeeder0Test, ResourceFixture) {
    Network network = ::powsybl::network::EurostagFactory::createTutorial1Network();
    ...
    ExtensionProviders<converter::xml::ExtensionXmlSerializer>::registerExtension("position", stdcxx::make_unique<ConnectablePositionXmlSerializer>());

    const std::string& networkStr = ResourceFixture::getResource("/extensions/cvg/connectablePositionFeeder0.xml");
    converter::RoundTrip::runXml(network, networkStr);
}

BOOST_FIXTURE_TEST_CASE(ConnectablePositionXmlSerializerFeeders123Test, ResourceFixture) {
    Network network = ::powsybl::network::EurostagFactory::createTutorial1Network();
    network.setCaseDate(stdcxx::DateTime::parse("2013-01-15T18:45:00.000+01:00"));
    Load& load = network.getLoad("LOAD");
    ...
    const std::string& networkStr = ResourceFixture::getResource("/extensions/cvg/connectablePositionFeeders123.xml");
    converter::RoundTrip::runXml(network, networkStr);
}
