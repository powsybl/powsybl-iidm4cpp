...
Network createNBKLcc() {
    Network network("nbklcc", "test");
...
BOOST_AUTO_TEST_CASE(testConnectDisconnect) {
    ...
    BOOST_CHECK(!hvdcLine.disconnectConverterStations( [](const Switch& switchObject) {
            return !SwitchPredicate::IS_NONFICTIONAL()(switchObject) && !SwitchPredicate::IS_OPEN()(switchObject);
    }));

    // Disconnection
    BOOST_CHECK(hvdcLine.disconnectConverterStations());
    BOOST_CHECK(!hvdcLine.getConverterStation1().get().getTerminal().isConnected());
    BOOST_CHECK(!hvdcLine.getConverterStation2().get().getTerminal().isConnected());
    ...
    BOOST_CHECK(!hvdcLine.connectConverterStations( [](const Switch& switchObject) {
    ...
    // Disconnect one side
    BOOST_CHECK(hvdcLine.disconnectConverterStations(SwitchPredicate::IS_CLOSED_BREAKER(), TwoSides::ONE));
    BOOST_CHECK(!hvdcLine.getConverterStation1().get().getTerminal().isConnected());
    BOOST_CHECK(hvdcLine.getConverterStation2().get().getTerminal().isConnected());

    // Connection on the other side fails since it's still connected
    BOOST_CHECK(!hvdcLine.connectConverterStations(SwitchPredicate::IS_NONFICTIONAL_BREAKER(), TwoSides::TWO));
