...
BOOST_AUTO_TEST_CASE(errors) {
    ...
    BOOST_CHECK(!IdBasedBusRef("unknown", TwoSides::ONE).resolve(network, TopologyLevel::BUS_BREAKER));
    BOOST_CHECK(!IdBasedBusRef("unknown", TwoSides::TWO).resolve(network, TopologyLevel::BUS_BREAKER));
...
BOOST_AUTO_TEST_CASE(test) {
    ...
    BOOST_CHECK(stdcxx::areSame(IdBasedBusRef("NGEN_NHV1", TwoSides::ONE).resolve(network, TopologyLevel::BUS_BREAKER).get(), network.getBusBreakerView().getBus("NGEN").get()));
    BOOST_CHECK(stdcxx::areSame(IdBasedBusRef("NGEN_NHV1", TwoSides::TWO).resolve(network, TopologyLevel::BUS_BREAKER).get(), network.getBusBreakerView().getBus("NHV1").get()));
...
BOOST_AUTO_TEST_CASE(busBranch) {
    ...
    BOOST_CHECK(stdcxx::areSame(IdBasedBusRef("NGEN_NHV1", TwoSides::ONE).resolve(network, TopologyLevel::BUS_BRANCH).get(), network.getBusView().getBus("VLGEN_0").get()));
    BOOST_CHECK(stdcxx::areSame(IdBasedBusRef("NGEN_NHV1", TwoSides::TWO).resolve(network, TopologyLevel::BUS_BRANCH).get(), network.getBusView().getBus("VLHV1_0").get()));
